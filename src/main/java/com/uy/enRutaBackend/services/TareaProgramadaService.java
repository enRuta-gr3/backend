...
public class TareaProgramadaService implements ITareaProgramadaService {
    ...
    public void cerrarVentasDeViajesProximos() {
        ...
        List<Viaje> viajes = viajeRepository.findViajesProximosPorEstado(
        ...
        for (Viaje viaje : viajes) {
            ...
            List<Pasaje> pasajes = pasajeRepository.findByViaje(viaje).stream()
                .filter(p -> p.getEstadoPasaje() == EstadoPasaje.VIGENTE &&
                             p.getVenta_compra() != null &&
                             "COMPLETADA".equals(p.getVenta_compra().getEstado()))
                .toList();
